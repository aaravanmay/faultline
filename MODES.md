...
```python
fl.instrument(graph_or_tools, actions=["place_order"])    # langgraph / langchain / llamaindex / pydantic-ai / crewai (idempotent)
# or be explicit: fl.instrument_langgraph(graph) / fl.instrument_langchain(tools) / fl.instrument_llamaindex(tools)
```
Verified against real installed **langgraph**, **langchain**, **llama-index**, **pydantic-ai**, and **crewAI** (`tests/test_*_real.py`),
